...
if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'medecin_interimaire') {
    header("Location: login.php");
    exit();
}

if (!isset($_GET['id'])) {
    header("Location: listespatient_interimaire.php");
    exit();
}
...
if (!$patient) {
    header("Location: listespatient_interimaire.php?error=Patient non trouvé");
    exit();
}

// Récupérer les historiques d'observations
$stmt_obs = $pdo->prepare("SELECT * FROM observations WHERE id_patient = ? ORDER BY date_observation DESC");
$stmt_obs->execute([$patient_id]);
$observations = $stmt_obs->fetchAll();

// Récupérer les ordonnances
$stmt_ord = $pdo->prepare("SELECT * FROM ordonnances WHERE id_patient = ? ORDER BY date_ordonnance DESC");
...
<?php endif; ?>
  ...
  <a href="voir_ordonance.php?id=<?= $consult['id'] ?>&date=<?= date('Y-m-d', strtotime($consult['date_consultation'])) ?>" 
     class="btn btn-outline-success" title="Voir ordonnance">
              ...
              <tr>
                ...
                <td>
                  <div class="btn-group btn-group-sm">
                    ...
                    <a href="imprimer_ordonnance.php?id=<?= $ord['id'] ?>" class="btn btn-outline-primary" title="Imprimer">
