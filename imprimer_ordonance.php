...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO ordonnances (id_patient, date_ordonnance, medicaments, posologie, duree_traitement, notes, statut) 
                VALUES (?, NOW(), ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$patient_id, $medicaments, $posologie, $duree_traitement, $notes, $statut]);
            
            $ordonnance_id = $pdo->lastInsertId();
            header("Location: voir_ordonnance.php?id=$ordonnance_id&success=Ordonnance créée avec succès");
            exit();
        } catch (Exception $e) {
            ...
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <select name="statut" class="form-select">
                  ...
                  <option value="suspendue" <?= ($_POST['statut'] ?? '') === 'suspendue' ? 'selected' : '' ?>>Suspendue</option>
