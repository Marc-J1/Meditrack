...
<div class="pc-container">
  <div class="pc-content">
    <div class="page-header">
      <h4 class="mb-3">Liste des Médecins</h4>
    </div>
              ...
              while ($row = $stmt->fetch()) {
                  echo "<tr>
                      ...
                      <td>{$row['statut']}</td>
                       <td>{$row['telephone']}</td>
                      <td>
                        <a href='modifier_medecin_interimaire.php?id_medecin={$row['id_medecin']}' class='btn btn-sm btn-primary' title='Modifier'><i class='ti ti-edit'></i></a>
                        <a href='supprimer_medecin_interimaire.php?id_medecin={$row['id_medecin']}' class='btn btn-sm btn-danger' title='Supprimer' onclick=\"return confirm('Confirmer la suppression ?');\"><i class='ti ti-trash'></i></a>
                      </td>
                  </tr>";
              }
              ?>
...
<script>
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href = 'supprimer_medecin_interimaire.php?id=' + id;
    }
  });
});
